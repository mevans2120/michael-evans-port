...
/**
 * BentoImageSide - Variation 3
 *
 * Side-by-side image and content layout.
 * Creates a balanced, magazine-style presentation with images as visual anchors.
 */
const BentoImageSide: React.FC<BentoImageSideProps> = ({ isDarkMode }) => {
  ...
  return (
    <section className="py-20 px-6 relative overflow-hidden">
      ...
      <div className={`absolute left-1/3 bottom-1/4 w-96 h-96 rounded-full blur-3xl ${
        isDarkMode ? 'bg-purple-500/10' : 'bg-purple-100 opacity-20'
      }`} />
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        <div className="mb-12 -mx-6 md:mx-0 text-center">
          ...
          <p className={`text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Magazine-style layout with images as visual anchors
          </p>
        </div>

        {/* Bento Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 auto-rows-[minmax(220px,auto)] gap-4 -mx-6 md:mx-0">
              ...
              >
                ...
                >
                  ...
                  <div className={`p-0 h-full flex ${
                    isLarge || isWide ? 'flex-row' : 'flex-col'
                  }`}>
                    {/* Image Section */}
                    <div className={`relative overflow-hidden ${
                      isLarge ? 'w-2/5' : isWide ? 'w-1/3' : 'h-32'
                    }`}>
                      <img
                        src={project.image}
                        alt={project.title}
                      ...
                      />
                      {/* Gradient overlay on smaller cards */}
                      {!isLarge && !isWide && (
                        <div className={`absolute inset-0 bg-gradient-to-b ${
                          isDarkMode ? 'from-transparent to-gray-900/60' : 'from-transparent to-white/60'
                        }`} />
                      )}
                    </div>

                    {/* Content Section */}
                    <div className={`flex-1 p-6 flex flex-col ${isLarge ? 'justify-between' : ''}`}>
                      {/* Category badge */}
                      <div className="mb-3">
                      ...
                      <p
                        className={`mb-4 leading-relaxed ${
                          isLarge ? 'text-base line-clamp-4' : isWide ? 'text-sm line-clamp-2' : 'text-sm line-clamp-2'
